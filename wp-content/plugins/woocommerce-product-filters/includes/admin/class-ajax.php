...
class Ajax extends Structure\Component {
    ...
    public function rules_builder_get_options() {
        ...
        if ( $param == 'category' || $param == 'tag' ) {
            $options = get_terms( [
                'taxonomy' => $param == 'category' ? 'product_cat' : 'product_tag',
                'fields' => 'id=>name',
                'count' => false,
            ...
            foreach ( get_taxonomies( [ 'object_type' => [ 'product' ] ], 'objects' ) as $taxonomy ) {
                if ( taxonomy_is_product_attribute( $taxonomy->name )
                    || in_array( $taxonomy->name, [ 'product_cat', 'product_tag', 'product_type' ] ) ) {
                    continue;
                }
    ...
    public function save_project_ajax() {
        $project_structure = isset( $_POST['projectEntity'] ) ? $_POST['projectEntity'] : null;
    ...
    public function control_reload_ajax() {
        ...
        $controls_props = isset( $_POST['controls'] ) ? $_POST['controls'] : null;
        ...
        foreach ( $controls_props as $item_index => $control_props ) {
            ...
            $response_data['items'][ $item_index ] = [
