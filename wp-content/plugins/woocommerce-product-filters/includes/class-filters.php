...
class Filters extends Structure\Component {
    protected $products_query_vars = [];

    protected $filter_markers = [];
    ...
    public function on_init() {
        if ( class_exists( 'WC_Photography_Products' ) ) {
            $has_handler = has_action( 'pre_get_posts', [ wc()->query, 'pre_get_posts' ] );

            if ( $has_handler ) {
                remove_action( 'pre_get_posts', [ wc()->query, 'pre_get_posts' ] );

                add_action( 'pre_get_posts', [ wc()->query, 'pre_get_posts' ], 15 );
            }
        }
    }
    ...
    public function end_of_buffering() {
        echo $this->replace_markers_in_buffer( ob_get_clean() );
    }
    ...
    public function save_query_and_apply_filters( $query ) {
         ...
         */
        $has_product_query_handler = has_action( 'pre_get_posts', [ wc()->query, 'product_query' ] );
        ...
        $this->products_query_vars[ $project_id ] = [
        ...
        if ( ( ( $out_of_stock_products == 'hide-if-active-any-options' && wcpf_is_filtered() )
                || $out_of_stock_products == 'always-hide' )
    ...
    public function print_notes_for_product_filters( $project_id, $echo = true ) {
        ...
        $result = $this->get_template_loader()->compile_template( 'notes-for-product-filters.php', [
    ...
    public function print_product_filters( $project_id, $echo = true, $attributes = [] ) {
    ...
    public function print_marker_for_product_filters( $project_id, $echo = true, $attributes = [] ) {
        $this->filter_markers[ $project_id ] = [
            'content' => '<!-- woocommerce-product-filters: ' . $project_id . ' -->',
            'before' => isset( $attributes['before_html'] ) ? $attributes['before_html'] : null,
            'after' => isset( $attributes['after_html'] ) ? $attributes['after_html'] : null
        ];

        if ( $echo ) {
            echo $this->filter_markers[ $project_id ]['content'];
        }
