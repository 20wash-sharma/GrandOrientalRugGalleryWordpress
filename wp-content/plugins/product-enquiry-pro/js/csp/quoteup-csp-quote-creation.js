...
(function($){
    $(document).ready(function(){
        $(document).on('change', '.newqty', function(){
            ...
            let user_email            = jQuery('input.wdm-input.input-email').val();
            let product_id            = 0;
            let price = 0.0;
        ...
        jQuery('.quoteup-csp-price-update-btn').click(function(){
            ...
            product_row.each(function(index, element){
                ...
                price      = quoteupCSPgetPriceFor(quantity, csp_prices[product_id]['prices']);
...
function quoteupFetchCSPPrice(product_id, asyncRequest = true, user_email = '',)
{
    ...
    if (0 == csp_prices.length || jQuery.isEmptyObject(csp_prices[product_id])) {
        if ('' == user_email) {
            user_email = jQuery('input.wdm-input.input-email').val();
        }
