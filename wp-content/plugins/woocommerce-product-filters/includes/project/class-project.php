...
class Project extends Structure\Component {
    ...
    protected $virtual_id_list = [];

    protected $entity_list = [];

    protected static $loaded_project = [];
    ...
    public function save_entity_by_structure( array $entity_structure ) {
        ...
        if ( $parent_id != 0 && get_post_status( $parent_id ) === false ) {
            $parent_id = 0;
        }
        ...
        $post_data = [
            ...
            'post_status' => 'publish',
            'meta_input' => [
                $this->get_option_key_meta() => isset( $entity_structure['options'] ) ? $entity_structure['options'] : [],
                $this->get_entity_key_meta() => $entity_structure['entityKey']
            ]
        ];

        if ( $entity_structure['status'] == 'virtual' ) {
        ...
        } else if ( $entity_structure['status'] == 'remove' ) {
        ...
        } else if ( $entity_structure['status'] == 'published' ) {
        ...
        if ( ! is_wp_error( $result ) && isset( $entity_structure['childEntities'] ) ) {
            foreach ( $entity_structure['childEntities'] as $child ) {
                ...
                if ( $entity_structure['status'] == 'remove' ) {
                    $child['status'] = 'remove';
                }
    ...
    public function save_project_by_structure( array $project_structure ) {
        ...
        try {
            ...
            if ( ! is_wp_error( $result ) ) {
                ...
                $this->get_hook_manager()->trigger_action( 'wcpf_save_project', $this, [
