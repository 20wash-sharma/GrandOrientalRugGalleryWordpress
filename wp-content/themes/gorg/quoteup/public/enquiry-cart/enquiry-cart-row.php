...
<tr class='cart_item cart_product'>
    ...
    <td class="product-name">
        ...
        <a href="<?php echo $url; ?>">
			...
			echo get_the_title(pll_get_post($product[ 'id' ])); // wdm code added
			?>
        </a>
    ...
    <td class='product-quantity'>
        <div class='quantity wdm-quantity'>
            <input type='number' min='1' step='1' data-product_id='<?php echo $product[ 'id' ]; ?>' data-variation_id='<?php echo $product['variation_id']; ?>' data-variation = '<?php echo $dataVariation; ?>' name='wdm_product_quantity' value='<?php echo $product[ 'quant' ]; ?>' class='wdm-prod-quant input-text qty '>
        </div>
    </td>
    <?php
