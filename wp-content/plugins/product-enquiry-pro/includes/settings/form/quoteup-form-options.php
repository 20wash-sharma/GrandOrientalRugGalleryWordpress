    ...
    <fieldset>
        ...
        <div class='fd'>
            ...
            <div class='right_div'>
                ...
                />
                <?php
                echo '<em>'.__('  Default Form ', QUOTEUP_TEXT_DOMAIN).'</em>';
                ?>
                <br>
                ...
                />
                <?php
                echo '<em>'.__(' Custom Form', QUOTEUP_TEXT_DOMAIN).'</em>';
                ?>
            </div>
            <div class="clear"></div>
        </div>
...
function displayCustomFormSettings($form_data)
{
    ...
    <?php echo '<legend>'.__('Custom Form Settings ', QUOTEUP_TEXT_DOMAIN).'</legend>'; ?>
        <div class="fd">
            <div class='left_div'>
                <label for="custom_formId"> <?php _e('Custom Form', QUOTEUP_TEXT_DOMAIN) ?> </label>
            </div>
            <div class='right_div'>
                <?php
                $linkNewForm=admin_url('post-new.php?post_type=form');
                $post_type = 'form';
                $select_id = "wdm_form_data[custom_formId]";
                $selected = isset($form_data['custom_formId'])? $form_data['custom_formId'] : 0;
                $posts = get_posts(array('post_type'=> $post_type, 'post_status'=> 'publish', 'suppress_filters' => false, 'posts_per_page'=>-1));
                echo '<select class="input-without-tip" name="'. $select_id .'" id="'.$select_id.'">';
                echo '<option value = "" >'. __('Please select form', 'quoteup') .'</option>';
                foreach ($posts as $post) {
                    echo '<option value="', $post->ID, '"', $selected == $post->ID ? ' selected="selected"' : '', '>', $post->post_title, '</option>';
                }
                echo '</select>'; ?>
                <a class="new_page_link" href="<?php echo $linkNewForm;
    ?>"> <?php _e('Add New Form', QUOTEUP_TEXT_DOMAIN); ?> </a>
            </div>
            <div class='clear'></div>
        </div >
        <div class="fd">
