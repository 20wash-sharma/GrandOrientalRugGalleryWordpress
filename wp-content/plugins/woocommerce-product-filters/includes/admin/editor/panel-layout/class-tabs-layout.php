...
class Tabs_Layout extends Abstract_Panel_Layout {
    ...
    public function add_tab( $tab_id, $tab_label, array $controls = [] ) {
        $this->panel_tabs[ $tab_id ] = [
            'label' => $tab_label,
            'controls' => $controls
        ];
    }
    ...
    public function add_control( $tab_id, Control\Abstract_Control $control, $position = false ) {
        if ( $this->has_tab( $tab_id ) ) {
            if ($position === false) {
                ...
                array_splice( $this->panel_tabs[ $tab_id ]['controls'], $position, 0, [ $control ] );
            }
        }
    }
    ...
    public function has_control( $tab_id, Control\Abstract_Control $control ) {
        if ( $this->has_tab( $tab_id ) ) {
            $key = array_search( $control, $this->get_tab_controls( $tab_id ) );
