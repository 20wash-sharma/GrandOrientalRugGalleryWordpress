...
class Plugin {
    ...
    protected function attach_hooks() {
        ...
        add_action( 'widgets_init', [ $this, 'on_widgets_init' ] );

        add_action( 'plugins_loaded', [ $this, 'plugin_support' ] );
    ...
    protected function is_active_woocommerce() {
        ...
        $is_active = in_array( $woocommerce, (array) get_option( 'active_plugins', [] ) );
    ...
    public function get_resource_url() {
        return $this->get_plugin_url() . 'assets/static/';
    }

    public function get_assets_url() {
        return $this->get_plugin_url() . 'assets/' . ( $this->is_debug_mode() ? 'dev' : 'prod' ) . '/';
    }

    public function get_assets_path() {
        return $this->get_plugin_path() . 'assets/' . ( $this->is_debug_mode() ? 'dev' : 'prod' ) . '/';
    }
