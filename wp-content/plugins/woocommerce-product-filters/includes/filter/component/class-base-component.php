...
class Base_Component extends Structure\Component {
    ...
    protected $child_filter_components = [];
    ...
    public function check_rules_from_option( $option_key, array $args = [] ) {
        global $wp_the_query;
        ...
        foreach ( $option_value as $group ) {
            ...
            foreach ( $rules as $rule_item ) {
                ...
                if ( in_array( $rule['param'], [ 'category', 'attribute', 'taxonomy', 'tag' ] ) ) {
                    ...
                    if ( $args['use_selected_options'] ) {
                        $selected_options = $this->get_object_register()->get( 'selected_options', [] );
                            ...
                            if ( $term instanceof \WP_Term ) {
                                foreach ( $selected_options as $selected_option ) {
                                    if ( $selected_option['taxonomy'] == $term->taxonomy
                                        && in_array( $term->slug, $selected_option['terms'] ) ) {
                                        $is_selected = true;
                    ...
                    $is_archive = $queried_object instanceof \WP_Term && $queried_object->term_id == $rule['value'];
                    ...
                    if ( $rule['value'] == $shop_id && is_shop() ) {
                        $current_page_id = $shop_id;
                    }

                    if ( ( $rule['operator'] == '==' && ( $current_page_id === false || $current_page_id != $rule['value'] ) )
                        || ( $rule['operator'] == '!=' && $current_page_id == $rule['value'] ) ) {
                        $and_result = false;
