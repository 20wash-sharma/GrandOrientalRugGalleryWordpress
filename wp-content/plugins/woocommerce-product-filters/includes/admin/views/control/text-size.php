<div class="control text-size-control control-inline-style"
    ...
    <div class="control-content">
        <div class="control-inputs-container">
            ...
            <select class="select-unit" name="<?php echo esc_attr( $option_key ) . 'Unit'; ?>" <?php if ( count( $units ) == 1 ): ?>disabled<?php endif; ?>>
                <?php foreach ( $units as $key_unit => $title_unit ): ?>
                    <option value="<?php echo esc_attr( $key_unit ); ?>"><?php echo $title_unit; ?></option>
                <?php endforeach; ?>
            </select>
        </div>
