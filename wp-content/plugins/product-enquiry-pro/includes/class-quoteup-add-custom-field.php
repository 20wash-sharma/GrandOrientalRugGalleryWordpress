...
class QuoteUpAddCustomField
{
    ...
    public function __construct()
    {
        ...
        add_action('mep_custom_fields', array($this, 'mpeCustomFieldDashboard'));
    ...
    public function mpeCustomFieldDashboard($enquiry_id)
    {
        ...
        foreach ($results as $key => $v) {
            ...
            $custom_field_data .= "<div class='wdm-user-custom-info' title='".esc_html($v->meta_value)."'>";
            $custom_field_data .= "<input type='text' value='".esc_html($v->meta_value)."' class='wdm-input-custom-info wdm-input' disabled>";
            $custom_field_data .= '<label placeholder="'.__($meta_key_name, QUOTEUP_TEXT_DOMAIN).'" alt="'.$meta_key_name.'"></label></div>';
            unset($key);
        }
